...
public class GhostValleyBiome extends SuperoresModElements.ModElement {
	public static Biome biome;

	public GhostValleyBiome(SuperoresModElements instance) {
		super(instance, 565);
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				BiomeAmbience effects = new BiomeAmbience.Builder().setFogColor(12638463).setWaterColor(4159204).setWaterFogColor(329011)
						.withSkyColor(7972607).withFoliageColor(-3368704).withGrassColor(-3368704).build();
				...
				mobSpawnInfo.withSpawner(EntityClassification.CREATURE, new MobSpawnInfo.Spawners(EntityType.SHEEP, 1, 4, 4));
				mobSpawnInfo.withSpawner(EntityClassification.MONSTER, new MobSpawnInfo.Spawners(EntityType.VEX, 20, 4, 4));
				biome = new Biome.Builder().precipitation(Biome.RainType.RAIN).category(Biome.Category.NONE).depth(0.1f).scale(0.2f)
						.temperature(1.7000000000000002f).downfall(1f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("superores:ghost_valley"));
			}
		}
	}
	...
	private static class CustomLeaveVineTreeDecorator extends LeaveVineTreeDecorator {
		...
		static {
			...
			tdt.setRegistryName("ghost_valley_lvtd");
	...
	private static class CustomTrunkVineTreeDecorator extends TrunkVineTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("ghost_valley_tvtd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
	...
	private static class CustomCocoaTreeDecorator extends CocoaTreeDecorator {
		...
		static {
			codec = com.mojang.serialization.Codec.unit(() -> instance);
			tdt = new TreeDecoratorType(codec);
			tdt.setRegistryName("ghost_valley_ctd");
			ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
		}
