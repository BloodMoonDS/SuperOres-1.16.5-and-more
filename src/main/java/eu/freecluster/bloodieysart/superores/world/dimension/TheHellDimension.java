...
import eu.freecluster.bloodieysart.superores.item.TheHellItem;
import eu.freecluster.bloodieysart.superores.block.MeatBlockBlock;
import eu.freecluster.bloodieysart.superores.block.CandyStoneBlockBlock;
...
public class TheHellDimension extends SuperoresModElements.ModElement {
	@ObjectHolder("superores:the_hell_portal")
	public static final CustomPortalBlock portal = null;

	public TheHellDimension(SuperoresModElements instance) {
		super(instance, 165);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(MeatBlockBlock.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("superores:meat_land")).getGenerationSettings().getSurfaceBuilder()
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("superores:the_hell"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("the_hell_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("the_hell_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("the_hell_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new TheHellItem().setRegistryName("the_hell"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("superores:the_hell"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("superores:the_hell")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == CandyStoneBlockBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? CandyStoneBlockBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, CandyStoneBlockBlock.block.getDefaultState(), 3);
					}
				}
			}
