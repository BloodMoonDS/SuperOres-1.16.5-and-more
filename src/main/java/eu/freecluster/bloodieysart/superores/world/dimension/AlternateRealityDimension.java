...
package eu.freecluster.bloodieysart.superores.world.dimension;
...
import eu.freecluster.bloodieysart.superores.item.AlternateRealityItem;
...
public class AlternateRealityDimension extends SuperoresModElements.ModElement {
	@ObjectHolder("superores:alternate_reality_portal")
	public static final CustomPortalBlock portal = null;

	public AlternateRealityDimension(SuperoresModElements instance) {
		super(instance, 222);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("superores:black_forest")).getGenerationSettings()
		...
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("forest")).getGenerationSettings().getSurfaceBuilder().get()
				.getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("superores:lemon_land")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("superores:lemon_land")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		...
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("jungle")).getGenerationSettings().getSurfaceBuilder().get()
				.getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("superores:corruped_forest")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("superores:corruped_forest")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("superores:alternate_reality"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("alternate_reality_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("alternate_reality_portal",
					Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("alternate_reality_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new AlternateRealityItem().setRegistryName("alternate_reality"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("superores:alternate_reality"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("superores:alternate_reality")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == Blocks.MOSSY_COBBLESTONE;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? Blocks.MOSSY_COBBLESTONE.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, Blocks.MOSSY_COBBLESTONE.getDefaultState(), 3);
					}
				}
			}
